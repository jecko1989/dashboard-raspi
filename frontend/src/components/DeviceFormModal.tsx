...
// Modale per modificare un device esistente (l'id è immutabile).
export function DeviceFormModal({ open, device, onClose, onSaved }: DeviceFormModalProps) {
  const { luoghi, loading: loadingLuoghi } = useLuoghi();
  const [form, setForm] = useState<FormState>({
    name: device.name,
    hostname: device.hostname,
    ip_vpn: device.ip_vpn,
    luogo_id: device.luogo_id,
  ...
  const validate = (): boolean => {
    ...
    if (!form.name.trim()) next.name = 'Il nome visualizzato è obbligatorio.';
    if (!HOSTNAME_RE.test(form.hostname.trim())) next.hostname = 'Hostname non valido.';
    if (!isValidIpVpn(form.ip_vpn.trim())) {
      next.ip_vpn = 'Usa un IPv4, un IPv6 oppure un nome host/MagicDNS.';
    }
    if (!form.luogo_id) next.luogo_id = 'Seleziona un luogo.';
    if (!SSH_USER_RE.test(form.ssh_username.trim())) {
      next.ssh_username = 'Utente SSH non valido.';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const saved = await updateDevice(device.id, payload);
      onSaved?.(saved);
      onClose();
    } catch (err) {
      ...
      else if (status === 409) setServerError('Hostname o indirizzo già in uso.');
      else if (status === 404) setServerError('Device o luogo inesistente.');
      ...
      <div className="my-8 w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-4">
          ...
          <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
            La chiave SSH e i campi di stato (online, latenza) non sono modificabili qui.
          </p>
