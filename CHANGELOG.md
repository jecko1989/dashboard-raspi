...
* **backend/frontend:** tempo di permanenza offline al posto dell'uptime quando un device non e' raggiungibile — nuova colonna `Device.offline_since` (impostata/azzerata in `metrics_service.check_and_collect` sulle transizioni di stato, esposta in `DeviceRead`); `DeviceCard` e `DeviceDetails` mostrano "Offline da" con `formatDurationSince` invece dell'uptime quando `is_online` e' `false`
