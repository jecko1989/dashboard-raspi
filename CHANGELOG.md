...
## [0.9.0](https://github.com/jecko1989/dashboard-raspi/compare/v0.8.0...v0.9.0) (2026-09-13)
...
* **devices:** mostra tempo offline al posto dell'uptime quando il device non e' raggiungibile ([e373c41](https://github.com/jecko1989/dashboard-raspi/commit/e373c410419a99c477eea3d26ae83ebf88e690e8))
...
* **fan-control:** implement automatic fan channel selection for multi-fan systems ([c4f832a](https://github.com/jecko1989/dashboard-raspi/commit/c4f832a66b8eafac52faede0361d308f5e8de448))
...
* **luogo-section:** add renderMode prop for flexible layout options ([87196df](https://github.com/jecko1989/dashboard-raspi/commit/87196dfee3aa06cd15182949a727b0f9a812c689))
...
* **commands:** usa DEBIAN_FRONTEND=noninteractive per update_upgrade ([ff91133](https://github.com/jecko1989/dashboard-raspi/commit/ff91133f0cc0a364c9be38767a4a22af3a5423e5))
...
* **frontend:** "Ultima volta online" al posto dell'uptime quando un device non e' raggiungibile — `DeviceCard` e `DeviceDetails` mostrano `formatDateTime(device.last_metric_at)` invece dell'uptime quando `is_online` e' `false` (riusa il campo esistente, aggiornato solo dopo una raccolta metriche riuscita)
* **backend:** selezione automatica del canale ventola/temperatura su device con piu' sensori hwmon (desktop multi-fan) — `fan_rpm`/`fan_mode` preferiscono il canale con RPM > 0 misurato ora invece del primo trovato; `dashboard-fan-control.sh` supporta un override esplicito (`/etc/default/dashboard-fan-control` con `PWM_CHIP_NAME`/`PWM_CHANNEL_INDEX`/`PWM_TEMP_SEL`) per i chip Super I/O desktop con piu' header ventola, dove l'auto-detect non puo' sapere quale sia collegato alla CPU (vedi `docs/DEPLOYMENT.md`)
* **frontend:** `LuogoSection` supporta `renderMode: 'inline'` — la pagina Overview usa un'unica griglia condivisa tra tutti i luoghi invece di una griglia per luogo, cosi' i luoghi con un solo device possono affiancarsi nella stessa riga invece di occupare una riga intera a testa
...
* **commands:** `update_upgrade` ora esegue `apt-get -y upgrade` con `DEBIAN_FRONTEND=noninteractive` — senza, una domanda debconf (es. charset console-setup o layout tastiera durante la configurazione di pacchetti come `console-setup`/`keyboard-configuration`) restava in attesa di un input che non arriva mai su una sessione SSH non interattiva, bloccando il lock dpkg a tempo indeterminato (il processo restava orfano se la sessione SSH si chiudeva). Richiede una nuova regola sudoers `env_keep` scoped al solo comando di upgrade (vedi README.md)
