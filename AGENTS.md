...
- Tempo offline (v0.8.0): `Device.offline_since` viene impostato in `metrics_service.check_and_collect` alla transizione online→offline e azzerato alla transizione offline→online; esposto in `DeviceRead`. Il frontend (`DeviceCard`, `DeviceDetails`) mostra "Offline da" (calcolato con `formatDurationSince`) al posto dell'uptime quando `is_online` e' `false`.
